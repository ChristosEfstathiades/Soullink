    ...
    return (
        <div>
            {Array.from({ length: 9 }).map((_, index) => (
                <div key={index} className={`inline-block rounded-xl cursor-pointer ${pcBoxBackground === index ? ' shadow-[0_0_25px_7px_rgba(255,28,28,1)]' : 'border-transparent'} m-2`} onClick={() => setPcBoxBackground(index)}>
