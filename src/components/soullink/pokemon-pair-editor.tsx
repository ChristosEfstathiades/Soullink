...
import { deletePair, updatePair } from '@/lib/storage';
...
export default function PokemonPairEditor({
    ...
    async function handleUpdatePair(event: React.FormEvent<HTMLFormElement>) {
        event.preventDefault();
        if (!pair) return;
        const form = event.currentTarget;
        const formData = new FormData(form);
        setProcessing(true);
        const result = await updatePair(save, pair, {
            ...
            isAlive: formData.get('isAlive') !== null,
        });
        setProcessing(false);
        if (result.ok) {
            setErrors({});
        ...
        return (
            <section className="">
                <div className="ml-2 flex flex-col items-center">
                    ...
                    <form className="flex flex-col items-center justify-around" onSubmit={handleUpdatePair}>
